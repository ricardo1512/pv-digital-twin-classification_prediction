    ...
    class DigitalTwinCracksTS(DigitalTwin):
        ...
        def run(self):
            ...
            for i, _ in enumerate(self.df.index):
                ...
                smooth_factor /= (1 - np.exp(-3))  # normalize to 0–1 range
           ...
           # Compute AC Voltages per phase and line (balanced assumption)
            phase_cols = ['a_u', 'b_u', 'c_u']
            line_cols = ['ab_u', 'bc_u', 'ca_u']
            output.loc[:, phase_cols] = 230 * np.tile(volt_factors[:, np.newaxis], (1, len(phase_cols)))
            output.loc[:, line_cols] = 400 * np.tile(volt_factors[:, np.newaxis], (1, len(line_cols)))
