    ...
    class DigitalTwinSoilingTS(DigitalTwin):
        ...
        def run(self):
            ...
            for i, _ in enumerate(self.df.index):
                ...
                self.soiling_factor = np.clip(self.soiling_factor, soiling_min, soiling_max)
...
# SÓ FUNCIONA PARA SUMMER !!!!!!!!!!!!!!!!!!!
    ...
    for file in os.listdir(WEATHER_FOLDER_PRED):
        ...
        for year in YEARS:
            ...
            twin = DigitalTwinSoilingTS(plant, inverter, ts_df, condition_nr, initial_soiling_factor)
            ...
            output_file = f"{TS_SAMPLES}/{condition_nr}_{condition_name}_{year}_{local}_ts_samples.csv"
